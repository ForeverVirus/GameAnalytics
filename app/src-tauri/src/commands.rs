...
use crate::graph::store::{GraphStore, FrontendGraph};
use crate::workspace;
use crate::analysis;
...
fn emit_progress(app: &tauri::AppHandle, phase: &str, step: &str, current: u32, total: u32, message: &str) {
...
fn async_cli_command(name: &str) -> TokioCommand {
    ...
    #[cfg(target_os = "windows")]
    {
        if let Ok(current_path) = std::env::var("PATH") {
            ...
            let new_path = format!("{};{}", extra_dirs.join(";"), current_path);
...
/// Build CLI arguments for AI analysis, including model and thinking level if configured
fn build_cli_args(cli_name: &str, model: &Option<String>, thinking: &Option<String>, codex_cd: Option<&str>) -> Vec<String> {
    match cli_name {
        "claude" => {
        ...
        "codex" => {
            ...
            // Prompt will be piped via stdin; add "-" as placeholder for codex positional arg
            args.push("-".to_string());
            args
        }
        _ => vec![],
...
#[tauri::command]
pub fn select_project(path: String, state: State<'_, AppState>) -> Result<workspace::ProjectInfo, String> {
...
pub fn has_analysis_cache(state: State<'_, AppState>) -> Result<bool, String> {
    ...
    if let Some(info) = project.as_ref() {
        let cache_path = PathBuf::from(&info.path).join(".analytics").join("cache.json");
...
fn build_file_structure_summary(file_id: &str, graph: &GraphStore) -> String {
    let mut classes: Vec<&GraphNode> = graph
        ...
        .filter(|n| {
            n.file_path.as_deref() == Some(file_id)
                && matches!(n.node_type, NodeType::Class | NodeType::Interface | NodeType::Module)
        })
        .collect();
    ...
    let mut members: Vec<&GraphNode> = graph
        .nodes
        .values()
        .filter(|n| n.file_path.as_deref() == Some(file_id) && n.node_type == NodeType::MemberVariable)
        .collect();
...
fn build_method_static_flow_summary(file_id: &str, source: &str, graph: &GraphStore) -> String {
    ...
    let outgoing_file_edges: Vec<&GraphEdge> = graph
        .edges
        .iter()
        .filter(|e| e.source == file_id && !matches!(e.edge_type, EdgeType::Contains | EdgeType::Declares))
        .collect();
    ...
    for (idx, method) in methods.iter().enumerate() {
        ...
        for (target_idx, target) in methods.iter().enumerate() {
            ...
            let Ok(pattern) = Regex::new(&format!(r"\b{}\s*\(", regex::escape(&target.name))) else {
                continue;
            };
    ...
    methods
        ...
        .map(|(idx, method)| {
            ...
            let hardcodes = graph
                .hardcode_findings
                .iter()
                .filter(|h| h.file_path == file_id && h.line_number >= method.line && h.line_number <= method.end_line)
...
#[tauri::command]
pub fn update_node_ai_summary(node_id: String, summary: String, analysis_type: String, state: State<'_, AppState>) -> Result<(), String> {
    let mut graph = state.graph.lock().map_err(|e| e.to_string())?;
    ...
    if let Some(node) = graph.nodes.get_mut(&target_node_id) {
        let existing = node.metadata.get("ai_summary")
...
#[tauri::command]
pub fn get_project_info(state: State<'_, AppState>) -> Result<Option<workspace::ProjectInfo>, String> {
    let project = state.project.lock().map_err(|e| e.to_string())?;
    Ok(project.clone())
}
...
#[tauri::command]
pub async fn run_analysis(app: tauri::AppHandle, state: State<'_, AppState>) -> Result<AnalysisStats, String> {
    let (project_path, engine) = {
        let project = state.project.lock().map_err(|e| e.to_string())?;
        let project_info = project.as_ref().ok_or("No project selected")?;
        (PathBuf::from(&project_info.path), project_info.engine.clone())
    };
    ...
    let files = tokio::task::spawn_blocking(move || {
        workspace::list_project_files(&pp, &eng)
    }).await.map_err(|e| e.to_string())?;

    let total_files = files.len();
    emit_progress(&app, "scan", "create_nodes", 1, 8, &format!("正在创建文件节点 ({} 个文件)...", total_files));
    ...
    match engine {
        EngineType::Unity => {
            emit_progress(&app, "scan", "guid_map", 2, 8, "正在构建 Unity GUID 映射...");
            let pp = project_path.clone();
            let guid_map = tokio::task::spawn_blocking(move || {
                analysis::build_unity_guid_map(&pp)
            }).await.map_err(|e| e.to_string())?;

            let unity_files: Vec<String> = files.iter()
                .filter(|f| {
                    let ext = f.rsplit('.').next().unwrap_or("").to_lowercase();
                    matches!(ext.as_str(), "prefab" | "unity" | "mat" | "asset" | "controller" | "overridecontroller" | "anim")
                })
                .cloned()
                .collect();
            let unity_total = unity_files.len();
            emit_progress(&app, "scan", "unity_refs", 3, 8,
        ...
        EngineType::Godot => {
            emit_progress(&app, "scan", "godot_refs", 2, 8, "正在分析 Godot 引用关系...");
        }
        _ => {
    ...
    let code_files: Vec<String> = files.iter()
        .filter(|f| f.ends_with(".cs") || f.ends_with(".gd"))
        .cloned()
        .collect();
    let code_total = code_files.len();
    emit_progress(&app, "scan", "code_refs", 4, 8,
        &format!("正在分析代码交叉引用 ({} 个文件)...", code_total));

    {
        let pp = project_path.clone();
        let cf = code_files.clone();
        let class_map = tokio::task::spawn_blocking(move || {
            analysis::build_class_map(&pp, &cf)
        }).await.map_err(|e| e.to_string())?;

        emit_progress(&app, "scan", "code_refs", 4, 8,
            &format!("正在匹配代码引用 — {} 个类, {} 个文件...", class_map.len(), code_total));
    ...
    emit_progress(&app, "scan", "code_structure", 5, 8,
        &format!("正在解析代码结构 ({} 个文件)...", code_total));
    {
        let pp = project_path.clone();
        let cf = code_files.clone();
        let (new_nodes, new_edges) = tokio::task::spawn_blocking(move || {
            analysis::parse_code_structure(&pp, &cf)
        }).await.map_err(|e| e.to_string())?;
    ...
    emit_progress(&app, "scan", "suspected", 7, 8,
        &format!("正在检测疑似引用 ({} 个文件)...", code_total));
    {
        let pp = project_path.clone();
        let cf = code_files.clone();
        let refs = tokio::task::spawn_blocking(move || {
            analysis::detect_suspected_references(&pp, &cf)
        }).await.map_err(|e| e.to_string())?;
...
pub fn get_asset_graph(state: State<'_, AppState>) -> Result<FrontendGraph, String> {
    ...
    let is_asset_side = |node_type: &NodeType| {
        matches!(node_type, NodeType::Asset | NodeType::Directory | NodeType::SceneObject)
    };
...
pub fn get_code_graph(state: State<'_, AppState>) -> Result<FrontendGraph, String> {
    ...
    let nodes: Vec<_> = full.nodes.into_iter()
        .filter(|n| code_types.contains(&n.node_type))
        .collect();
    let node_ids: std::collections::HashSet<&str> = nodes.iter().map(|n| n.id.as_str()).collect();
    let edges: Vec<_> = full.edges.into_iter()
...
#[tauri::command]
pub async fn run_ai_analysis(node_id: String, cli_name: String, model: Option<String>, thinking: Option<String>, app: tauri::AppHandle, state: State<'_, AppState>) -> Result<String, String> {
    ...
    let args = build_cli_args(&cli_name, &model, &thinking, codex_cd);

    emit_ai_log(&app, &format!("[快速分析] 请求节点: {}", node_id));
    emit_ai_log(&app, &format!("  实际分析文件: {}", target_node_id));
    emit_ai_log(&app, &format!("  CLI: {} | prompt长度: {} 字符", cli_name, prompt.len()));
    ...
    let mut child = cmd.spawn()
        .map_err(|e| format!("无法运行 {}: {}。请确保已安装该 CLI 工具并加入 PATH。", cli_name, e))?;

    // Write prompt via stdin to avoid cmd.exe command-line length limits on Windows
    if let Some(mut stdin_pipe) = child.stdin.take() {
        stdin_pipe.write_all(prompt.as_bytes()).await
...
#[tauri::command]
pub async fn run_deep_ai_analysis(node_id: String, cli_name: String, model: Option<String>, thinking: Option<String>, app: tauri::AppHandle, state: State<'_, AppState>) -> Result<String, String> {
    let (prompt, target_node_id, runtime_dir) = {
        let graph = state.graph.lock().map_err(|e| e.to_string())?;
        let project = state.project.lock().map_err(|e| e.to_string())?;
        let project_path = project.as_ref().map(|p| p.path.clone()).unwrap_or_default();
...
fn is_analyzable_node(node: &GraphNode) -> bool {
    matches!(node.node_type,
        NodeType::CodeFile | NodeType::Asset | NodeType::Class |
        NodeType::Method | NodeType::Interface | NodeType::Module |
        NodeType::SceneObject
    )
}

fn build_ai_prompt(project_root: &str, requested_node_id: &str, target_node_id: &str, graph: &GraphStore) -> String {
...
fn build_batch_prompt(dir: &str, nodes_in_dir: &[&GraphNode], graph: &GraphStore) -> String {
    ...
    for node in nodes_in_dir {
        ...
        let outgoing: Vec<String> = graph.edges.iter()
        ...
        let mut entry = format!(
            ...
            node.name,
            node.node_type,
            node.asset_kind.as_ref().map(|k| format!("{:?}", k)).unwrap_or("无".to_string()),
        );

        if !incoming.is_empty() {
            entry.push_str(&format!("  被引用({}):\n{}\n", incoming.len(), incoming.join("\n")));
        }
        if !outgoing.is_empty() {
            entry.push_str(&format!("  引用({}):\n{}\n", outgoing.len(), outgoing.join("\n")));
        }

        // Suspected refs for this node
        let suspected: Vec<String> = graph.suspected_refs.iter()
            .filter(|sr| sr.code_location == node.id || sr.resource_path == node.id)
            .take(5)
            .map(|sr| format!("    动态加载: {} → {} ({})", 
                sr.code_location.rsplit('/').next().unwrap_or(&sr.code_location),
                sr.resource_path, sr.load_method))
            .collect();
        if !suspected.is_empty() {
            entry.push_str(&format!("  疑似引用({}):\n{}\n", suspected.len(), suspected.join("\n")));
        }

        // Hardcodes for this node
        let hardcodes: Vec<String> = graph.hardcode_findings.iter()
            .filter(|h| h.file_path == node.id)
            .take(5)
            .map(|h| format!("    L{}: {:?} \"{}\"", h.line_number, h.category, h.value))
            .collect();
        if !hardcodes.is_empty() {
            entry.push_str(&format!("  硬编码({}):\n{}\n", hardcodes.len(), hardcodes.join("\n")));
        }
...
fn build_deep_ai_prompt(project_root: &str, requested_node_id: &str, target_node_id: &str, graph: &GraphStore) -> String {
    let file_node = match graph.get_node(target_node_id) {
        Some(n) => n,
        None => return format!("Analyze node: {}", target_node_id),
    };
...
#[tauri::command]
pub async fn run_ai_batch_analysis(cli_name: String, model: Option<String>, thinking: Option<String>, app: tauri::AppHandle, state: State<'_, AppState>) -> Result<u32, String> {
    ...
    let dir_batches: Vec<(String, Vec<String>)> = {
        ...
        for node in graph.nodes.values() {
            if !is_analyzable_node(node) { continue; }
            let dir = node.id
                .rfind('/')
    ...
    emit_ai_log(&app, &format!("开始 AI 批量分析，共 {} 个目录", total_batches));

    for (idx, (dir, node_ids)) in dir_batches.iter().enumerate() {
        emit_progress(&app, "ai", &format!("batch_{}", idx), idx as u32, total_batches,
            &format!("AI 分析目录: {} ({}/{})", dir, idx + 1, total_batches));
        emit_ai_log(&app, &format!("\n━━━ [{}/{}] 分析目录: {} ━━━", idx + 1, total_batches, dir));
        ...
        if num_chunks > 1 {
            emit_ai_log(&app, &format!("  目录包含 {} 个节点，拆分为 {} 批", node_ids.len(), num_chunks));
        }
        ...
        for (chunk_idx, chunk_node_ids) in chunks.iter().enumerate() {
            ...
            let (prompt, node_name_map) = {
                let graph = state.graph.lock().map_err(|e| e.to_string())?;
                let nodes_in_dir: Vec<&GraphNode> = chunk_node_ids.iter()
                    .filter_map(|id| graph.get_node(id))
                    .collect();
                if num_chunks > 1 {
                    emit_ai_log(&app, &format!("  [批次 {}/{}] 处理 {} 个节点...", chunk_idx + 1, num_chunks, nodes_in_dir.len()));
                } else {
                    emit_ai_log(&app, &format!("  目录包含 {} 个节点", nodes_in_dir.len()));
                }
                let p = build_batch_prompt(dir, &nodes_in_dir, &graph);
                let names: Vec<(String, String)> = nodes_in_dir.iter()
            ...
            let codex_abs_dir = if cli_name == "codex" {
                PathBuf::from(&project_path).join(dir).to_string_lossy().to_string()
            } else {
                String::new()
            };
            let codex_cd = if !codex_abs_dir.is_empty() { Some(codex_abs_dir.as_str()) } else { None };
            let args = build_cli_args(&cli_name, &model, &thinking, codex_cd);

            emit_ai_log(&app, &format!("  调用 {} CLI... (prompt: {} 字符)", cli_name, prompt.len()));

            let result = match run_cli_streaming(&cli_name, &args, &prompt, &app, &project_path).await {
            ...
            {
                let name_refs: Vec<(&str, &str)> = node_name_map.iter()
                ...
                for node_id in *chunk_node_ids {
                    let summary = per_node.get(node_id.as_str())
                        .cloned()
    ...
    emit_ai_log(&app, &format!("\n━━━ AI 分析完成，成功 {}/{} 个目录 ━━━", analyzed, total_batches));
    emit_progress(&app, "ai", "done", total_batches, total_batches, "AI 分析完成");
    Ok(analyzed)
}

/// Run a CLI command asynchronously, streaming stdout/stderr lines as ai_log events.
async fn run_cli_streaming(cli_name: &str, args: &[String], prompt: &str, app: &tauri::AppHandle, project_dir: &str) -> Result<String, String> {
    let mut cmd = async_cli_command(cli_name);
    ...
    let status = child.wait().await.map_err(|e| format!("等待进程结束失败: {}", e))?;
    ...
    if status.success() {
        ...
        Err(format!("CLI 返回错误 (exit {}): {}", status.code().unwrap_or(-1), err.trim()))
    }
}

/// Export analysis results to .analytics/ directory
#[tauri::command]
pub fn export_analysis(app: tauri::AppHandle, state: State<'_, AppState>) -> Result<String, String> {
    let project = state.project.lock().map_err(|e| e.to_string())?;
    let project_info = project.as_ref().ok_or("No project selected")?;
    let graph = state.graph.lock().map_err(|e| e.to_string())?;
    ...
    let suspected_json = serde_json::to_string_pretty(&graph.suspected_refs).map_err(|e| e.to_string())?;
    std::fs::write(export_dir.join("suspected-refs.json"), &suspected_json).map_err(|e| e.to_string())?;

    // Export hardcode-findings.json
    let hardcode_json = serde_json::to_string_pretty(&graph.hardcode_findings).map_err(|e| e.to_string())?;
    std::fs::write(export_dir.join("hardcode-findings.json"), &hardcode_json).map_err(|e| e.to_string())?;
        ...
        if let Some(summary) = node.metadata.get("ai_summary") {
            if summary != "未分析" {
                let dir = node.id
                    ...
                    .map(|i| node.id[..i].to_string())
                    .unwrap_or_else(|| "root".to_string());
                dir_summaries.entry(dir).or_default().push(
                    format!("### {}\n{}\n", node.name, summary)
                );
            }
        }
    ...
    let asset_nodes: Vec<&GraphNode> = graph.nodes.values()
        .filter(|n| n.node_type == NodeType::Asset)
        .collect();
    let referenced_targets: std::collections::HashSet<&str> = graph.edges.iter()
        .map(|e| e.target.as_str())
        .collect();
    let unused: Vec<&&GraphNode> = asset_nodes.iter()
